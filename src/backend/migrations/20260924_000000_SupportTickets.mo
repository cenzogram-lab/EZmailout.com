/// Adds the `supportTickets` inbox filled by the Stampy support drawer.
/// A new stable variable starts empty; every existing one is carried over
/// unchanged, so no record is re-typed.
...
module {
  ...
  // ─── OLD (equal to NewActor of 20260923_000000_DispatchProcessing) ───
  ...
  public func migration(old : OldActor) : NewActor {
    {
      ...
      supportTickets = List.empty<SupportTicket>();
    };
  };
};
