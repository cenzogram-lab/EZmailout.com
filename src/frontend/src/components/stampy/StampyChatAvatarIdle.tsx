...
/**
 * Stampy chat avatar at rest, with a notification dot.
 *
 * Generated from `stampy-chat-avatar.svg`. Clip-path and gradient ids are prefixed per
 * instance, so any number of these can share a page.
 */
export function StampyChatAvatarIdle({ title, ...props }: StampyProps) {
  ...
  return (
    ...
    >
      ...
      <defs>
        <clipPath id={`${uid}-chat-avatar-avClip1`}>
          <circle cx="32" cy="32" r="31" />
        </clipPath>
        <clipPath id={`${uid}-chat-avatar-bodyClip`}>
          <path d="M124 390 C118 300 150 256 200 256 C250 256 282 300 276 390 Q200 404 124 390 Z" />
        </clipPath>
      </defs>
      <g>
        <circle cx="32" cy="32" r="31" fill="#F4F4F4" />
        <g clipPath={`url(#${uid}-chat-avatar-avClip1)`}>
          <g transform="translate(-8 -2) scale(0.2)">
            <g>
              <g
                stroke="#16204A"
                strokeWidth="6"
