...
/**
 * Stampy chat avatar while a reply is being written; the three dots pulse (`.stampy-dot`, see index.css).
 *
 * Generated from `stampy-chat-avatar-thinking.svg`. Clip-path and gradient ids are prefixed per
 * instance, so any number of these can share a page.
 */
export function StampyChatAvatarThinking({ title, ...props }: StampyProps) {
  ...
  return (
    ...
    >
      ...
      <defs>
        <clipPath id={`${uid}-chat-avatar-thinking-avClip2`}>
          <circle cx="32" cy="32" r="31" />
        </clipPath>
        <clipPath id={`${uid}-chat-avatar-thinking-bodyClip`}>
          <path d="M124 390 C118 300 150 256 200 256 C250 256 282 300 276 390 Q200 404 124 390 Z" />
        </clipPath>
      </defs>
      <g>
        <circle cx="32" cy="32" r="31" fill="#F4F4F4" />
        <g clipPath={`url(#${uid}-chat-avatar-thinking-avClip2)`}>
          <g transform="translate(-11 0) scale(0.2)">
            <g>
              <g
                stroke="#16204A"
                strokeWidth="6"
