...
- Outcall transforms: `Http.transform` drops headers; an outcall whose success body is only volatile ids passes `transformContext = Http.maskSuccessBody` so a 2xx body is blanked and every replica sees the same response.
